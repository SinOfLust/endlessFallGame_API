...
const queries: {
    getSkins(root: any, { category }: { category: string }): Promise<string[]>,
    account(root: any, { _id, }: { _id: string }): Promise<mongoose.Document[]>,
    getLevel(): Promise<number>
} = {
    // resolver to fetch files URL within public/skins directory from a GraphQL query
    getSkins: async ({ category }: { category: folder }, root?: any): Promise<string[]> => {
